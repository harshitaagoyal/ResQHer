...
export default function AdminIncidentTable({ incidents, onViewDetails, onDeleteSuccess }) {
  ...
  // 🚨 NEW: Sort State
  const [sortDirection, setSortDirection] = useState(null); // 'desc', 'asc', or null

  // 🚨 NEW: Click Outside Logic to close the dropdown
  useEffect(() => {
  ...
  // 🚨 NEW: Sort Logic for Severity
  const handleSort = () => {
    if (sortDirection === null) setSortDirection('desc'); // First click: Highest to Lowest
    else if (sortDirection === 'desc') setSortDirection('asc'); // Second click: Lowest to Highest
    else setSortDirection(null); // Third click: Turn off sort
  };

  // Helper dictionary to give severities a number value so they can be sorted mathematically
  const severityRank = { "Very High": 4, "High": 3, "Medium": 2, "Low": 1 };

  const sortedIncidents = [...incidents].sort((a, b) => {
    if (!sortDirection) return 0; // Don't sort if direction is null
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-white dark:bg-slate-900 text-slate-500 border-b border-slate-200 dark:border-slate-800">
            <tr>
              ...
              {/* 🚨 UPDATED: Clickable Sort Header */}
              <th className="px-6 py-4 font-medium">
                <button 
                  onClick={handleSort}
                  className="flex items-center gap-1 hover:text-slate-900 dark:hover:text-white transition-colors outline-none"
                >
                  Severity 
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {/* 🚨 UPDATED: Map over sortedIncidents instead of the raw ones */}
              ...
              <tr key={incident._id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors bg-white dark:bg-slate-900">
                ...
                <td className="px-6 py-4 font-medium text-slate-900 dark:text-white">
                  <button 
                    ...
                    className="hover:text-pink-600 dark:hover:text-pink-400 hover:underline focus:outline-none transition-colors text-left font-bold"
                  >
                    {incident.name}
                  </button>
                </td>
                ...
                <td className="px-6 py-4 text-slate-600 dark:text-slate-400">
                  {typeof incident.location === 'object' ? "GPS Logged" : incident.location || "Tamil Nadu"}
                </td>
                ...
                <td className="px-6 py-4 text-slate-600 dark:text-slate-400 text-center">
                  <p className="max-w-[200px] truncate mx-auto">
                    {incident.currentSituation || incident.other_info || "Emotional"}
                  </p>
                </td>
                ...
                <td className="px-6 py-4 text-right relative">
                  {/* 🚨 UPDATED: Stop click propagation so clicking this specific button doesn't instantly close the menu via the window click listener */}
                  <button 
                    onClick={(e) => { 
                      e.stopPropagation(); 
                      setOpenDropdown(openDropdown === incident._id ? null : incident._id); 
                    }} 
                    className="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 rounded transition-colors text-slate-500"
                  >
                  ...
                  {openDropdown === incident._id && (
                    ...
                    >
                      ...
                      <button onClick={() => { navigator.clipboard.writeText(incident._id); setOpenDropdown(null); }} className="w-full text-left px-4 py-2 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700">Copy ID</button>
                      <button onClick={() => { onViewDetails(incident); setOpenDropdown(null); }} className="w-full text-left px-4 py-2 text-sm text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 border-b dark:border-slate-700">View Details</button>
                      <button 
                        onClick={() => handleDelete(incident._id)}
                        disabled={isDeleting}
                        className="w-full flex items-center justify-between px-4 py-2 text-sm text-red-600 dark:text-red-400 font-medium hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                      >
