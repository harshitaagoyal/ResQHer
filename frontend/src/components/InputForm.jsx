...
import { Loader2, LocateIcon, UploadCloud, Image as ImageIcon, X, CheckCircle2 } from 'lucide-react';
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-xl border-2 border-dashed border-slate-200 dark:border-slate-700">
          ...
          <div className="flex items-center gap-4 mt-2">
            <Input
              className="bg-white dark:bg-slate-950 dark:border-slate-800 text-sm"
            ...
            <Button
              ...
              variant="outline"
              className="shrink-0 gap-2 border-pink-200 text-pink-700 hover:bg-pink-50 dark:border-pink-900/50 dark:text-pink-400 dark:hover:bg-pink-950/30 font-semibold transition-colors"
            >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mt-3">
              {attachments.map((file, idx) => (
                <div key={idx} className="relative flex items-center justify-between bg-slate-100 dark:bg-slate-800 p-2.5 rounded-lg border border-slate-200 dark:border-slate-700">
                  ...
                  <button type="button" onClick={() => removeFile(idx)} className="p-1 hover:bg-red-200 dark:hover:bg-red-900/40 text-red-500 rounded-md transition-colors shrink-0 ml-2">
                    <X className="h-4 w-4" />
                  </button>
                </div>
              ))}
        ...
        <Button
          type="submit"
          disabled={loading}
          className="w-full bg-pink-600 hover:bg-pink-700 dark:bg-pink-700 dark:hover:bg-pink-600 text-white h-12 text-lg font-bold transition-all mt-4"
        >
          {loading ? (
